...
-o $ORDERER -C mychannel -n bearing_supply_chain -v v1.0.3 \
-c '{"Args":["init"]}' \
...
docker exec cli peer chaincode list --instantiated -o $ORDERER \
		-C mychannel --cafile /opt/home/managedblockchain-tls-chain.pem --tls
...
docker exec -e "CORE_PEER_TLS_ENABLED=true" -e "CORE_PEER_TLS_ROOTCERT_FILE=/opt/home/managedblockchain-tls-chain.pem" \
    ...
    cli peer chaincode query -C mychannel -n bearing_supply_chain -c '{"Args":["queryBearing","{\"UID\": \"d8a83c3eeer3werw\"}"]}'
